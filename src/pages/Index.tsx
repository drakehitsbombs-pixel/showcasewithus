    ...
    <div className="page-frame">
      ...
      <header className={`navbar ${navSolid ? "is-solid" : ""}`}>
        ...
        <nav className="hidden md:flex items-center gap-4 flex-shrink-0">
          <Link
            to="/discover"
            className="text-sm font-semibold hover:text-cp-green transition-colors whitespace-nowrap"
          >
            Find Photographers
          </Link>
        ...
        <div className="flex items-center gap-2 absolute left-1/2 transform -translate-x-1/2">
          ...
          <span className={`text-lg md:text-xl font-bold tracking-tight ${navSolid ? "text-cp-ink" : "text-white"}`}>
            SHOW CASE
          </span>
        </div>
      ...
      <section className="mammut-hero" aria-label="Intro">
        <div className="mammut-media">
          <video className="mammut-video" autoPlay muted loop playsInline>
        ...
        <div className="mammut-content">
          ...
          <h1 className="display">
            <span className="soft">FIND YOUR</span>
            <br />
            <span className="strong">PERFECT MATCH</span>
          </h1>
          <p className="sub">Browse, compare, and book your weddings, surf, portraits and more.</p>
          <div className="cta-row">
            <button onClick={() => navigate("/discover")} className="btn btn-primary">
              I am looking for a photographer
            </button>
            <button onClick={() => navigate("/auth")} className="btn btn-ghost">
              I am a photographer
            </button>
          </div>
          <p className="mt-4 text-white/80 text-sm">Free to browse • No signup to view profiles</p>
        </div>
      ...
      >
        <div className="grid md:grid-cols-2 gap-6 max-w-6xl mx-auto">
          ...
          >
            ...
            <button onClick={() => navigate("/discover")} className="btn-primary btn-md">
              View Photographers
            </button>
          </div>
          ...
          >
            ...
            <button onClick={() => navigate("/auth")} className="btn-secondary btn-md">
              Get Started
            </button>
          </div>
        </div>
